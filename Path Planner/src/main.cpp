...
bool isFrontClose(double car_s, double ego_s){
  double distance = abs(car_s - ego_s);
  return (ego_s>car_s) && (distance < bufferDistance);
}

bool isBackClose(double car_s, double ego_s){
  double distance = abs(car_s - ego_s);
  return  (ego_s < car_s) && (distance < bufferDistance);
}
  ...
  double car_d_dash = ego_car_points[1];
   ...
   //if car's d is within the current lane's range
    if (isLane(car_d_dash)) {
      //if there's a car infront in the current lane --> currentt cost += 1
      if(isFrontClose(car_s,car_s_dash)) {
        currentCost += dist_cost(distance, front_weight); // penalize more for cars infront 
      }
    }
    //if the left lane is valid, and car's d is within left lane range
    if (isLeftLane(car_d_dash)) {
      //if there's a car infront or behind in the left lane --> left lane cost += 1
      if(isFrontClose(car_s,car_s_dash)) {
        leftCost += dist_cost(distance,front_weight) + 0.2; //penalize for switching lanes
      }
      if(isBackClose(car_s_dash,car_s)){
        leftCost += dist_cost(distance,back_weight) +0.2;
      }
    }
    //same as the left lane for the right lane
    if (isRightLane(car_d_dash)) {
      if(isFrontClose(car_s,car_s_dash)){
        rightCost += dist_cost(distance,front_weight) + 0.2; // penalize for switching lanes
      }
       if(isBackClose(car_s_dash,car_s)){
        rightCost += dist_cost(distance,back_weight) +0.2;
      }
    }
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          tk::spline s;
          s.set_points(ptsx,ptsy);#include <uWS/uWS.h>
#include <fstream>
#include <iostream>
#include <string>
#include <vector>
